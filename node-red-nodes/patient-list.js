...
module.exports = function(RED) {
    ...
    function getProperty(node,msg,done) {
        if (node.propertyType === 'jsonata') {
            RED.util.evaluateJSONataExpression(node.property,msg,(err,value) => {
                if (err) {
                    done(RED._("patient-list.errors.invalid-expr",{error:err.message}));
                } else {
    ...
    function SwitchNode(n) {
        ...
        if (this.propertyType === 'jsonata') {
            try {
                ...
                this.error(RED._("patient-list.errors.invalid-expr",{error:err.message}));
                return;
            }
        }
        ...
        function addMessageToGroup(id, msg, parts) {
            ...
            if ((max_msgs > 0) && (pendingCount > max_msgs)) {
                clearPending();
                node.error(RED._("patient-list.errors.too-many"), msg);
            }
    ...
    RED.nodes.registerType("patient-list", SwitchNode);
}
